...
// 혁명 전체 화면 오버레이
function showRevolutionBanner(nickname, avatar) {
  const el = document.createElement("div");
  el.style.cssText = `
  ...
  `;
  el.innerHTML = `
    <div style="font-size:72px;animation:spin 0.6s ease-out;">${avatar}</div>
    <div style="font-size:40px;margin:12px 0 4px;color:#C9A227;font-family:Georgia,serif;">⚡ 혁명!</div>
    <div style="font-size:18px;color:#F2E8D5;">${nickname}님이 혁명을 선언했습니다!</div>
    <div style="font-size:14px;color:#C9A227;margin-top:8px;">세금 징수가 취소됩니다</div>
  `;

  if (!document.getElementById("revStyle")) {
    const s = document.createElement("style");
    s.id = "revStyle";
    s.textContent = `
      @keyframes fadeInOut { 0%{opacity:0} 15%{opacity:1} 75%{opacity:1} 100%{opacity:0} }
      @keyframes spin { from{transform:rotate(-30deg)scale(0.5)} to{transform:rotate(0)scale(1)} }
    `;
...
function renderWaitingRoom() {
  ...
  app.innerHTML = `
    ...
    <p style="font-size:12px;color:var(--gold);opacity:.8;">
      참가 인원 ${room.players.length} / ${room.maxPlayers} (최소 ${room.minPlayers}명)
    </p>

    ${isHost ? `
    <div class="actions" style="margin-bottom:8px;">
      <button id="addBotBtn" class="btn" style="flex:1;" ${room.players.length >= room.maxPlayers ? "disabled" : ""}>🤖 봇 추가</button>
      <button id="removeBotBtn" class="btn" style="flex:1;" ${botCount === 0 ? "disabled" : ""}>➖ 봇 제거</button>
    </div>` : ""}

    <div class="actions">
      <button id="readyBtn" class="btn" style="flex:1;">${me?.isReady ? "준비 취소" : "준비완료"}</button>
      ${isHost ? `<button id="startBtn" class="btn btn-gold" style="flex:1;" ${canStart ? "" : "disabled"}>게임 시작</button>` : ""}
    </div>
  ...
  room.players.forEach((p) => {
    ...
    div.innerHTML = `
      ...
      <span class="name">${p.nickname}${p.userId === room.hostUserId ? " 👑방장" : ""}${p.isBot ? " 🤖" : ""}</span>
      <span class="status ${p.isReady ? "ready" : ""}">${p.isBot ? "봇 (자동)" : p.connected === false ? "연결 끊김" : p.isReady ? "준비완료" : "대기중"}</span>
    `;
  ...
  document.getElementById("leaveBtn").onclick  = () => { if (confirm("방에서 나가시겠습니까?")) leaveRoom(); };

  if (isHost) {
    document.getElementById("addBotBtn").onclick    = () => socket.emit("room:addBot");
    document.getElementById("removeBotBtn").onclick = () => socket.emit("room:removeBot");
    document.getElementById("startBtn").onclick     = () => socket.emit("game:start");
    const closeBtn = document.getElementById("closeBtn");
    closeBtn.onclick = () => {
      if (confirm("방을 삭제하면 모든 플레이어가 강제 퇴장됩니다. 삭제하시겠습니까?")) socket.emit("room:close");
    };
  }
...
function renderTax() {
  ...
  document.getElementById("leaveBtn").onclick = () => {
    if (confirm("게임 도중 나가면 대기실로 초기화됩니다. 나가시겠습니까?")) leaveRoom();
  };
...
function renderGameTable() {
  ...
  room.players.forEach((p) => {
    ...
    const isTurn  = trick?.currentTurnUserId === p.userId;
    const div     = document.createElement("div");
    div.className = "player-card"
      + (p.rank?.isKing ? " king" : "")
      + (p.rank?.isPeon ? " peon" : "")
    ...
    div.innerHTML = `
      ...
      <span class="avatar">${p.avatar}</span>
      <span class="name">${p.nickname}${p.isBot ? " 🤖" : ""}</span>
  ...
  if (!trick || trick.fieldCards.length === 0) {
    ...
    trick.fieldCards.forEach((fc) => {
      ...
      c.textContent = `${fc.rank === 13 ? "🃏" : fc.rank}×${fc.count}`;
...
socket.on("connect", () => {
  if (state.userId && state.roomCode) {
    socket.emit("room:join", {
      ...
      avatar: state.avatar,    userId: state.userId,
    }, (res) => {
      if (!res?.ok) {
        state.roomCode = null;
        localStorage.removeItem("dalmuti_roomCode");
...
socket.on("game:hand",   ({ hand }) => { state.myHand = hand; state.selected.clear(); render(); });
...
socket.on("turn:passed", ({ userId, isTimeout }) => {
  const name = state.room?.players?.find(p => p.userId === userId)?.nickname || "플레이어";
  if (isTimeout) showToast(`⏱ ${name}님 시간 초과 — 자동 패스`);
});

socket.on("player:finished", ({ userId, place }) => {
  const name = state.room?.players?.find(p => p.userId === userId)?.nickname || "플레이어";
  if (userId === state.userId) showToast(`🎉 카드를 모두 냈습니다! ${place}등으로 마감`);
  else showToast(`${name}님 ${place}등으로 마감`);
});

// 혁명 전체 화면 이벤트
socket.on("tax:revolution", ({ userId, nickname, avatar }) => {
  showRevolutionBanner(nickname, avatar);
  if (userId === state.userId) showToast("혁명 선언! 세금 징수를 건너뜁니다", 3000);
});
...
socket.on("room:hostChanged",   ({ nickname }) => showToast(`👑 ${nickname}님이 새 방장이 되었습니다`));
socket.on("room:resetToLobby",  ({ reason })   => { state.myHand = []; state.selected.clear(); showToast(reason); });

socket.on("room:closed", ({ reason }) => {
  showToast(reason || "방이 삭제되었습니다");
  state.room = null; state.roomCode = null; state.myHand = []; state.selected.clear();
  rulesAutoShown = false;
  localStorage.removeItem("dalmuti_roomCode");
  clearInterval(timerInterval);
  render();
